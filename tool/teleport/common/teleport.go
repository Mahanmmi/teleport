...
func Run(options Options) (app *kingpin.Application, executedCommand string, conf *service.Config) {
	...
	dumpNodeConfigure.Flag("join-method", "Method to use to join the cluster (token, iam, ec2, oidc-gcp)").Default("token").EnumVar(&dumpFlags.JoinMethod, "token", "iam", "ec2", "oidc-gcp")
