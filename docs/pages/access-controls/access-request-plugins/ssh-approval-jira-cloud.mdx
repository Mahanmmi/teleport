...
<TabItem label="Helm Chart" scope={["oss","enterprise","cloud"]}>
```yaml
(!examples/resources/plugins/teleport-jira-helm-self.yaml!)
```

The YAML file above contains the settings for the Helm Chart, the `teleport`, `jira`, and `log` sections
follow the corresponding sections of the plugin's TOML config structure. See the
[teleport-plugin-jira Helm Chart Reference](../../setup/helm-reference/teleport-plugin-jira.mdx) for more
detailed reference.
