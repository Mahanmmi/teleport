...
Teleport's PagerDuty Access Request Plugin has its own configuration
file in TOML format. Before starting the plugin for the first time,
you'll need to generate and edit that config file.
...
By default, `teleport-pagerduty` will assume its config is in
...
```code
$ helm install teleport-plugin-pagerduty teleport/teleport-plugin-pagerduty --values teleport-pagerduty-helm.yaml
```
</TabItem>
</Tabs>
...
The PagerDuty Access Request Plugin has an option to auto-approve Access
Requests. This feature will map an external SSO identity to a PagerDuty
on-call email address. If the user requesting matches the person on call
