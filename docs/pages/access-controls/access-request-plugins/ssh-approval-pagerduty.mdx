...
<Details title="Installing via Helm?">
  ...
  See an example for `teleport-pagerduty-helm.yaml` under [Running Teleport in Kubernetes?](#running-teleport-in-kubernetes)
</Details>
...
```console
kubectl create secret generic teleport-plugin-pagerduty-identity --from-file=auth_id=auth.pem
```
